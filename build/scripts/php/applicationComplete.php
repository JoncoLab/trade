...
foreach ($rows as $row) {
    ...
    if (in_array($lotId, $selectedLots)) {
        $sheet->removeRow($row);
    }
}
$writer = new PHPExcel_Writer_Excel2007($excel);
$writer->save(str_replace('scripts/php', '', __DIR__) . 'docs/' . $traderId . '.xlsx');

$filename = str_replace('scripts/php', '', __DIR__) . 'docs/' . $traderId . '.xlsx';
$p = "\r\n";
$to = $email;
...
$message .= 'Вашу заявку успішно опрацьовано.' . $p;
...
$multipartMessage .= 'Content-Disposition: attachment; filename="Заявка.xlsx"' . $p . $p;
