...
<header>
    <nav class="navbar">
        ...
        <button onclick="window.location.replace('../scripts/php/logout.php');">Вийти</button>
    </nav>
</header>
<main>
    <section class="settings page-maker">
        ...
        <form method="post" id="set-timer">
            <label for="timer">Задати час на крок (сек): </label>
            <input type="number" name="timer" id="timer" min="15" max="59" value="<?php print $settings["timer"];?>" required>
            <button type="submit" id="timer-submit">Підтвердити</button>
        </form>
        <form method="post" id="set-to">
            <label for="to">Кому надсилаються системні листи (через кому):</label>
            <textarea id="to" name="to" maxlength="5000" cols="150" rows="1"><?php print $settings["to"];?></textarea>
            <button type="submit" id="to-submit">Підтвердити</button>
        </form>
    </section>
    ...
    <table class="users page-maker">
        ...
        <tr>
            ...
            <th class="access">Доступ до торгів</th>
        ...
        while ($userId = $users->fetch_assoc()) {
            ...
            echo
                '<tr>' .
                '<td class="delete">' .
                '<button class="delete-button">Видалити користувача</button>' .
                ($user->ver == 1 ?
                    '<button class="cancel-verification-button">Анулювати верифікацію</button>' :
                    '<button disabled class="cancel-verification-button">Анулювати верифікацію</button>') .
                '</td>' .
                ...
                '<td class="access">' .
                ($user->ver == 0 ?
                    '<button disabled class="allow-access">Допустити</button>' :
                    ($user->access == 0 ?
                        '<button class="allow-access">Допустити</button>' :
                        '<button class="deny-access">Не допускати</button>')) .
                '</td>' .
